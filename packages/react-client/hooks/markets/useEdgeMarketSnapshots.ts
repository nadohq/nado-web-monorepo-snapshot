import { IndexerMarketSnapshot } from '@nadohq/client';
...
import { useQueryEdgeMarketSnapshots } from '../query/useQueryEdgeMarketSnapshots';
...
export function useEdgeMarketSnapshots({
      ...
      Object.entries(queryEdgeMarketSnapshotsData).forEach(
        ([chainEnv, snapshots]) => {
          snapshots.forEach((snapshot, index) => {
            ...
            // type (e.g. `openInterestsQuote`) with values of either type `BigNumber`
            // or `Record<number, BigNumber>`. So we build the snapshot with an object
  ...
  return {
    data: edgeMarketSnapshotsData,
    dataUpdatedAt,
    isLoading: isLoading || isLoadingQueryEdgeMarketSnapshotsData,
  };
}

function aggregateRecordData(
  existingData: Record<number, BigNumber>,
  newData: Record<number, BigNumber>,
) {
