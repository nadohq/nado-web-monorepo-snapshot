import { IndexerMarketSnapshot, nowInSeconds } from '@nadohq/client';
import { useQuery } from '@tanstack/react-query';
import { BigNumber } from 'bignumber.js';
import { get, mapValues } from 'lodash';
import { useNadoClientContext } from '../../context/nadoClient';
...
export function useQueryEdgeMarketSnapshots({
  ...
  const disabled = !primaryChainNadoClient || disabledOverride;
    ...
    queryFn: async (): Promise<EdgeMarketSnapshotsData> => {
      ...
      while (remainingLimit > 0) {
        const edgeMarketSnapshotsResponse =
          await primaryChainNadoClient.client.market.getEdgeMarketSnapshots({
            granularity,
        ...
        let oldestTimestamp: BigNumber | undefined;
