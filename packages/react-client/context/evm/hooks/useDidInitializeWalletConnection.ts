...
import type { GetConnectionReturnType } from 'wagmi/actions';

const PENDING_ACCOUNT_STATUSES: GetConnectionReturnType['status'][] = [
  'connecting',
  'reconnecting',
];
 ...
 * - When `isClient` is initially switched to `true`, `useConnection` status can still be in a `reconnecting`/`connecting` state.
 */
export function useDidInitializeWalletConnection() {
  const isClient = useIsClient();
  const { status: accountStatus } = useConnection();
