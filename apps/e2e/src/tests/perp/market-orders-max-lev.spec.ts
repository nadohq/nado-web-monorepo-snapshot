...
const MARKET_ORDER_TEST_CASES: MarketOrderTestCase[] = SIDES.flatMap((side) =>
  MARGIN_MODES.map((marginMode) => buildTestCase(side, marginMode)),
);

test.describe('Market Orders - Max Leverage', () => {
  ...
  for (const testCase of MARKET_ORDER_TEST_CASES) {
    test(`opens ${testCase.name}`, async () => {
