...
export function useExecuteRevokeSocialAccount() {
  ...
  const mutationFn = useExecuteInValidContext(
    useCallback(async (params: Params, context: ValidExecuteContext) => {
      ...
      return context.nadoClient.context.indexerClient.revokeSocialAccount({
  ...
  return useMutation({
    ...
    onError(error, variables) {
      logExecuteError('RevokeSocialAccount', error, variables);
    },
  });
}
