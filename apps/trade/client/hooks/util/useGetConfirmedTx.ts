...
export function useGetConfirmedTx() {
  ...
  return useCallback(
    async (txHashPromise: Promise<string>) => {
      if (!publicClient) {
        throw new Error('[useGetConfirmedTx] No public client available');
      }

      const hash = await txHashPromise;
      if (!isHex(hash)) {
        throw new Error(
          `[useGetConfirmedTx] Invalid transaction hash: ${hash}`,
        );
      }
      ...
      if (transactionReceipt.status !== 'success') {
        throw new Error('[useGetConfirmedTx] Rejected transaction');
      }
