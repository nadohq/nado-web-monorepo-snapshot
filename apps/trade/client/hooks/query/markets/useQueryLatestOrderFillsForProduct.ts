import { ChainEnv, GetIndexerMatchEventsParams } from '@nadohq/client';
import {
  calcOrderFillPrice,
  createQueryKey,
  QueryDisabledError,
 ...
 * We multiply by 2 because the base query returns both taker & maker, and we
 * filter out maker fills.
 *
 ...
 */
export const LATEST_ORDER_FILLS_LIMIT = 75 * 2;
...
export function useQueryLatestOrderFillsForProduct<
...
>({
  ...
  const queryFn = async () => {
    ...
    const baseData =
      await nadoClient.context.indexerClient.getMatchEvents(params);
    ...
    return baseData
      .filter((event) => {
        ...
        // For non-USDT pairs, filtering on `product_id` will check both base & quote balances, so on a wETH-USDT market with an additional
        // mETH-wETH market, a fill on mETH-wETH will actually come back in the query for wETH-USDT. As such, we need to filter these events out
        ...
        return !isRfq && isCorrectProduct && event.isTaker;
      })
