import { ChainEnv, toBigInt } from '@nadohq/client';
import {
  createQueryKey,
  QueryDisabledError,
  useEVMContext,
 ...
 * @returns
 */
export function useQueryMarkedWithdrawPoolIdxs({
  submissionIndices,
}: {
  ...
  const disabled = !nadoClient || !submissionIndices;
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const baseResponse =
        await nadoClient.context.contracts.withdrawPool.read.checkMarkedIdxs([
