...
/**
 * Gets or creates the user's referral code by hitting /fuul-get-or-create-code
 * on first load or subaccount change
 */
export function AutoGetOrCreateAddressFuulReferralCode() {
  ...
  const { mutate } = useMutation({
    mutationFn: async () => {
      ...
      console.debug(
        '[AutoGetOrCreateAddressFuulReferralCode] Getting or creating referral code for user',
      );

      const mutationParams: GetOrCreateFuulReferralCodeParams = {
        address,
      };
      const response = await fetch('/api/fuul-get-or-create-code', {
    ...
    onSuccess: (data) => {
      console.debug(
        '[AutoGetOrCreateAddressFuulReferralCode] Referral code resolved for user',
        data,
      );
      if (data.created) {
        refetchQueries();
