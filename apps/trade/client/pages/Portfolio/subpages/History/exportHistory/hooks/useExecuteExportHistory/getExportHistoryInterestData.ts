...
export async function getExportHistoryInterestData(
  ...
  queryLoop: while (true) {
    const response =
      await nadoClient.context.indexerClient.getPaginatedSubaccountInterestFundingPayments(
        {
          ...
          startCursor,
          productIds: [
            allMarketsStaticData.primaryQuoteProduct.productId,
            ...allMarketsStaticData.spotMarketsProductIds,
          ],
        },
      );
    ...
    for (const event of response.interestPayments) {
      ...
      items.push({
        ...
        asset,
        annualRateFrac: event.annualPaymentRate.toString(),
        interestPaymentAmount: interestPaymentAmount.toString(),
      });
    }
