...
function cumulativeAccountEquityUsdDataKey(data: PortfolioChartDataItem) {
  return data.portfolioValueUsd;
}

export function PortfolioOverviewAccountEquityChart({
  data,
}: PortfolioChartComponentProps) {
  ...
  const renderTooltipBodyContent: PortfolioChartTooltipBodyRenderFn =
    useCallback(
      ({ portfolioValueUsd }: PortfolioChartDataItem) => {
        return (
          <ChartTooltip.Item
            title={t(($) => $.accountEquity)}
  ...
  return (
    <ResponsiveContainer>
      <AreaChart data={data}>
        ...
        <Area
          {...AREA_CHART_DEFAULTS}
          dataKey={cumulativeAccountEquityUsdDataKey}
          stroke={getTradeAppColorVar('positive')}
          fill={PORTFOLIO_CHART_GRADIENT_URLS.accountEquity}
          activeDot={{
            ...CHART_DOT_DEFAULTS,
            stroke: getTradeAppColorVar('positive'),
          }}
