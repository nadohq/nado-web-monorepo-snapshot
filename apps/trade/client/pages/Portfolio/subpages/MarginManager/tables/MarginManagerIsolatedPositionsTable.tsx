...
const columnHelper =
  createColumnHelper<MarginManagerIsolatedPositionsTableItem>();

export function MarginManagerIsolatedPositionsTable({
  className,
}: WithClassnames) {
  ...
  const {
    positions: { isolatedMargin },
    isLoading,
  } = useMarginManagerPerpPositionsTable();

  const isConnected = useIsConnected();

  const columns: ColumnDef<MarginManagerIsolatedPositionsTableItem, any>[] =
    useMemo(() => {
      return [
          ...
          cell: (context) => (
            <TableCell>
              <PerpPositionLabel
                ...
                isoLeverage={context.row.original.iso.leverage}
        ...
        columnHelper.accessor('iso', {
          header: ({ header }) => (
            <HeaderCell
              header={header}
              definitionTooltipId="isolatedPerpPositionsMargin"
            >
              {t(($) => $.margin)}
            </HeaderCell>
          ),
          cell: (context) => {
            const { marginUsd, leverage, subaccountName } = context.getValue();
            return (
              <PerpMarginCell
                margin={{
                  marginModeType: 'isolated',
                  crossMarginUsedUsd: undefined,
                  isoMarginUsedUsd: marginUsd,
                  isoLeverage: leverage,
                }}
                isoSubaccountName={subaccountName}
              />
            );
          },
          enableSorting: false,
        ...
        columnHelper.display({
          ...
          header: ({ header }) => (
            <CalculatorIconHeaderCell
              definitionTooltipId="marginManagerPerpIsolatedMarginCalc"
              header={header}
            />
          ),
          cell: (context) => {
            const { productId, iso } = context.row.original;
            return (
                ...
                <SecondaryButton
                  ...
                  onClick={() => {
                    show({
                      ...
                      params: {
                        productId,
                        isoSubaccountName: iso.subaccountName,
                        isLimitOrder: true,
                      },
                    });
                  }}
    ...
    }, [isConnected, show, handleMarketClosePosition, t]);
