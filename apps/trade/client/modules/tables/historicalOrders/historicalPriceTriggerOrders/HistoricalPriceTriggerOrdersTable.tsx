...
const columnHelper = createColumnHelper<HistoricalPriceTriggerOrderTableItem>();

export function HistoricalPriceTriggerOrdersTable({
  pageSize,
  productIds,
  reduceOnly,
  showPagination,
  ...
  const { mappedData, isLoading, pagination } =
    useHistoricalPriceTriggerOrdersTable({
      pageSize,
      productIds,
      reduceOnly,
    });

  const columns: ColumnDef<HistoricalPriceTriggerOrderTableItem, any>[] =
    useMemo(() => {
      return [
          ...
          cell: (context) => (
            <TableCell>
              <OrderTypeLabel
                ...
                priceTriggerCriteria={context.row.original.priceTriggerCriteria}
              />
            </TableCell>
          ),
          sortingFn: 'basic',
