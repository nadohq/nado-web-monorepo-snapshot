...
import { useHistoricalIndividualTradesTable } from 'client/modules/tables/historicalTrades/historicalIndividualTrades/useHistoricalIndividualTradesTable';
...
export function HistoricalIndividualTradesTable({
  pageSize,
  showPagination,
  productIds,
}: Props) {
  ...
  const { mappedData, isLoading, pagination } =
    useHistoricalIndividualTradesTable({
      pageSize,
      productIds,
    });
  ...
  const columns: ColumnDef<HistoricalTradesTableItem, any>[] = useMemo(() => {
    return [
      ...
      columnHelper.accessor('timeFilledMillis', {
        header: ({ header }) => (
