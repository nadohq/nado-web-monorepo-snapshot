...
interface Params {
  ...
  drawOrderLines: () => Promise<unknown>;
  drawPositionLines: () => Promise<unknown>;
}

/**
 * NOTE: The previous implementation of this had some fancy logic for handling the widget being ready, but it was
 * never really bulletproof. For the sake of simplicity, we just catch all draw rejections and ignore errors.
 */
export function useDrawChartLinesWhenReady({
  ...
  // The draw functions are async (TV v29+ Promise APIs), so use `.catch()` on the
  // returned Promise — a sync `try/catch` would not catch rejections.
  useEffect(() => {
    if (!isWidgetAndSymbolReady) {
      return;
    }
    drawOrderLines().catch((err) => {
      console.debug(
