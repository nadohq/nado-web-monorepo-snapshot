...
import { XStocksPromoCard } from 'client/modules/trading/components/AccountInfoCard/XStocksPromoCard';
...
export function AccountInfoCard({ className, productId }: Props) {
  ...
  const isXStocksProduct = getIsXStocksProduct(productId);

  const { getIsZeroFeesMarketForSubaccount } =
    useGetIsZeroFeesMarketForSubaccount();
  const isZeroFeesMarket = productId
    ? getIsZeroFeesMarketForSubaccount(productId)
    : false;
      ...
      },
      {
        ...
        valueContent: (
          <div className="flex items-center gap-x-2">
            {isZeroFeesMarket && <ZeroFeesPromoPill />}
            <DefinitionTooltip
  ...
  }, [subaccountOverview, isZeroFeesMarket, feeRates, t]);
  ...
  return (
    ...
    >
      ...
      {isXStocksProduct && <XStocksPromoCard />}
    </TradingPageCard>
  );
}
