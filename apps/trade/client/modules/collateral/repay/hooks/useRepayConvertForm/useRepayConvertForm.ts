...
export function useRepayConvertForm({
  ...
  // Watched form state
  const [repayAmountInput, repayProductIdInput, sourceProductIdInput] =
    useWatch({
      control: form.control,
      name: ['repayAmount', 'repayProductId', 'sourceProductId'],
    });
  ...
  const validRepayAmount = useMemo(() => {
    return safeParseForData(positiveBigNumberValidator, repayAmountInput);
  }, [repayAmountInput]);
  ...
  const validateRepayAmount = useCallback(
    (
      ...
      const amount = safeParseForData(positiveBigNumberValidator, amountInput);
      if (!amount) {
        return 'invalid_input';
      }

      if (sizeIncrement && !isValidIncrementAmount(amount, sizeIncrement)) {
    ...
    [sizeIncrement, maxRepaySizeIgnoringAmountBorrowed],
  );

  // Global form error state
  const formError: RepayConvertFormErrorType | undefined = useMemo(() => {
    if (selectedRepayProduct?.decimalAdjustedNadoBalance.isPositive()) {
  ...
  const sourceAmount = useMemo(() => {
    ...
    return roundToDecimalPlaces(convertedAmount, 6, BigNumber.ROUND_UP);
  ...
  const onAmountBorrowingClicked = useCallback(() => {
    ...
    form.setValue(
      'repayAmount',
      roundToString(selectedRepayProduct.amountBorrowed, 8, BigNumber.ROUND_UP),
      {
        shouldValidate: true,
        shouldTouch: true,
      },
  ...
  return {
    ...
    sourceAmountValueUsd:
      selectedSourceProduct &&
      sourceAmount?.multipliedBy(selectedSourceProduct.oraclePrice),
    repayAmountValueUsd:
      selectedRepayProduct &&
      validRepayAmount?.multipliedBy(selectedRepayProduct.oraclePrice),
