...
import { useQueryMarkedWithdrawPoolIdxs } from 'client/hooks/query/withdrawPool/useQueryMarkedWithdrawPoolIdxs';
...
export function useAreWithdrawalsProcessing({
  ...
  const { data: markedWithdrawPoolIdxsData } = useQueryMarkedWithdrawPoolIdxs({
    submissionIndices,
  });
