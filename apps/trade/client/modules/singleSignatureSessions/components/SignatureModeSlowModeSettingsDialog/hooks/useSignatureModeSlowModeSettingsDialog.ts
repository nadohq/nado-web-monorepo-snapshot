...
export function useSignatureModeSlowModeSettingsDialog(): UseSignatureModeSlowModeSettingsDialog {
  ...
  const slowModeTxState = useQueryOnChainTransactionState({
  ...
  const [selectedMode, privateKey] = useWatch({
    control: form.control,
    name: ['selectedMode', 'privateKey'],
  });
    ...
    async (values: SignatureModeSlowModeSettingsFormValues) => {
      if (userAction === 'requires_fee_approval') {
        ...
        dispatchNotification({
          ...
          data: {
            errorNotificationTitle: t(($) => $.errors.approveFeeFailed),
            executionData: {
              txHashPromise,
            },
          },
      ...
      if (mutationError) {
        dispatchNotification({
          type: 'action_error_handler',
          data: {
            errorNotificationTitle: t(($) => $.errors.updateLinkedSignerFailed),
            executionData: {
