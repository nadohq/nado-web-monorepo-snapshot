 ...
 * The hue values start at 270° (purple) and progress incrementally based on the index.
 * The hue wraps around if it exceeds 360°, ensuring continuous cycling of colors.
 *
 * - Hue is calculated using the formula: `(index * incrementDeg + 270) % 360`
...
function generateChartFillColor(index: number, incrementDeg: number): string {
  // Increment hue based on the index and starting at 270° (purple), cycling within 0-360°.
  const hue = (index * incrementDeg + 270) % 360;
